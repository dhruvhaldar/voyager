...
## 2026-07-01 - [FastAPI Request Headers Mapping Overhead]
**Learning:** In high-frequency code paths like `RateLimiter` middleware, repeatedly accessing `request.headers.get()` dynamically instantiates a `Headers` mapping object in Starlette/FastAPI. This involves unpacking and mapping the underlying raw ASGI `scope["headers"]` tuples, adding significant allocation overhead (~0.28µs -> ~0.08µs per request when avoided). Iterating over `request.scope["headers"]` directly to find a specific key (like `x-forwarded-for`) is roughly ~3.5x faster.
**Action:** When inspecting specific headers in an endpoint or middleware that executes extremely frequently (e.g., rate limiters polled at 1Hz+), avoid `request.headers` and iterate `request.scope["headers"]` directly to reduce object allocation.
