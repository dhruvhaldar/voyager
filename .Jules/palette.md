## 2025-05-20 - Semantic Forms for Better Mobile and Accessibility UX
**Learning:** When injecting dynamic inputs (like an API key prompt) using standalone `<input>` and `<button>` elements, mobile keyboards may not correctly display the "Go" action, and password managers might fail to recognize the input natively. Using custom `keydown` event listeners for the "Enter" key is a brittle approach that doesn't trigger standard form submission semantics.
**Action:** Always wrap dynamically injected inputs and buttons within a semantic `<form>` element. Attach a `submit` event listener to the form and call `e.preventDefault()`, which naturally handles the "Enter" key, displays the correct mobile keyboard actions, and plays nicely with browser password managers.
