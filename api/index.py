...
async def verify_api_key(request: Request, api_key: str = Security(api_key_header)):
    if not api_key or not secrets.compare_digest(api_key, VOYAGER_API_KEY):
        ...
        logging.warning(f"AUDIT: Failed authentication attempt from IP: {client_ip}")
        raise HTTPException(
...
def get_client_ip(request: Request) -> str:
    """Extract real IP behind reverse proxies safely."""
    client_ip = None
...
def command_reboot(request: Request):
    client_ip = get_client_ip(request)
    logging.info(f"AUDIT: OBC Reboot command executed by IP: {client_ip}")
    obc.reboot()
...
def command_freeze(request: Request):
    client_ip = get_client_ip(request)
    logging.info(f"AUDIT: OBC Freeze command executed by IP: {client_ip}")
    obc.freeze()
